...
public final class MineboxItemDataUtils {
    ...
    private static final String[] EQUIPMENT_TOKENS = {
            "helmet", "chestplate", "leggings", "boots",
            "ring", "belt", "necklace", "pet", "back",
            "gloves"
    };
    ...
    private static final Map<CustomData, CachedItemData> ITEM_DATA_CACHE = new WeakHashMap<>();
     ...
     */
    public static Optional<CompoundTag> getCustomData(ItemStack stack) {
     ...
     */
    public static Optional<String> getItemId(CompoundTag nbt) {
     ...
     * @return {@code true} when the display-only marker is set
     */
    public static boolean isDisplayOnlyItem(CompoundTag nbt) {
     ...
     */
    public static Optional<CompoundTag> getPersistentData(CompoundTag nbt) {
     ...
     * @return non-empty persistent data compound when available
     */
    public static Optional<CompoundTag> getPersistentData(ItemStack stack) {
     ...
     * @return immutable persistent item metadata when available
     */
    public static Optional<PersistentItemData> getPersistentItemData(CompoundTag nbt) {
        Optional<String> itemId = getItemId(nbt);
        Optional<CompoundTag> persistent = getPersistentData(nbt);
     ...
     */
    public static boolean hasValue(CompoundTag nbt, String keyPath) {
     ...
     */
    public static Optional<Tag> getValue(ItemStack stack, String keyPath) {
     ...
     * @return resolved NBT element when present
     */
    public static Optional<Tag> getValueFromPersistent(ItemStack stack, String keyPath) {
     ...
     * @return resolved NBT element when present
     */
    public static Optional<Tag> getValue(CompoundTag nbt, String keyPath) {
     ...
     * @return string value when present
     */
    public static Optional<String> getStringValue(CompoundTag nbt, String keyPath) {
        if (nbt == null || keyPath == null || keyPath.isBlank()) {
            return Optional.empty();
        }

        String[] parts = splitKeyPath(keyPath);
        CompoundTag parent = getParentCompound(nbt, parts);
        if (parent == null) {
            return Optional.empty();
        }
     ...
     * @return integer value when present
     */
    public static Optional<Integer> getIntValue(CompoundTag nbt, String keyPath) {
        if (nbt == null || keyPath == null || keyPath.isBlank()) {
            return Optional.empty();
        }
     ...
     */
    public static Optional<CompoundTag> getCompoundValue(ItemStack stack, String keyPath) {
     ...
     * @return nested compound when present
     */
    public static Optional<CompoundTag> getCompoundValueFromPersistent(ItemStack stack, String keyPath) {
     ...
     * @return nested compound when present
     */
    public static Optional<CompoundTag> getCompoundValue(CompoundTag nbt, String keyPath) {
        if (nbt == null || keyPath == null || keyPath.isBlank()) {
            return Optional.empty();
        }
     ...
     */
    public static Optional<Integer> getCurrentDurability(CompoundTag persistent) {
     ...
     */
    public static Optional<CompoundTag> getStatsData(CompoundTag persistent) {
     ...
     * @return stats compound when present and non-empty
     */
    public static Optional<CompoundTag> getStatsData(ItemStack stack) {
     ...
     * Matches whole token segments so {@code watering_can} is not treated as a ring.
     *
     * @param itemId Minebox item identifier
    ...
    public static boolean isEquipmentOrAccessory(String itemId) {
        ...
        String padded = "_" + itemId + "_";
        for (String token : EQUIPMENT_TOKENS) {
            if (padded.contains("_" + token + "_") || padded.contains("_" + token)) {
                return true;
            }
        }
        return false;
    ...
    private static CompoundTag getParentCompound(CompoundTag root, String[] parts) {
        CompoundTag current = root;
        for (int i = 0; i < parts.length - 1; i++) {
            Optional<CompoundTag> child = current.getCompound(parts[i]);
            if (child.isEmpty()) {
                return null;
            }
            current = child.get();
    ...
    private static CachedItemData getCachedItemData(ItemStack stack) {
        ...
        CachedItemData cachedData = ITEM_DATA_CACHE.get(customDataComponent);
        if (cachedData != null) {
            return cachedData;
        }
        ...
        Optional<CompoundTag> persistentData = getPersistentData(customData);
        ...
        Optional<CompoundTag> statsData = persistentData.flatMap(MineboxItemDataUtils::getStatsData);
        ...
        ITEM_DATA_CACHE.put(customDataComponent, itemData);
        return itemData;
    }

    private record CachedItemData(
            Optional<CompoundTag> customData,
            Optional<String> itemId,
            boolean displayOnly,
            Optional<CompoundTag> persistentData,
            Optional<PersistentItemData> persistentItemData,
            Optional<Integer> currentDurability,
            Optional<CompoundTag> statsData
    ) {
    }
     ...
     */
    public record PersistentItemData(CompoundTag customData, String itemId, CompoundTag persistentData) {
    }
}
